...
import { CIRCUITS, CircuitData } from '../../data/circuits';
import { CircuitTrack } from '../calendar/CircuitTrack';
import { cn } from '../../lib/utils';
import { Compass, Info, Hash } from 'lucide-react';

function LazyCircuitCard({ circuit, index }: { circuit: CircuitData; index: number }) {
  ...
  return (
    <div className="bg-slate-900 border border-white/10 rounded-3xl p-5 flex flex-col gap-4 group hover:border-blue-500/30 transition-all duration-300 shadow-xl relative overflow-hidden">
      ...
      <div ref={containerRef} className="w-full h-48 bg-slate-950 rounded-2xl flex items-center justify-center border border-white/5 relative overflow-hidden group-hover:border-blue-500/20 transition-colors">
        ...
        {isInView ? (
           ...
           <div className="w-full h-48 bg-slate-950 rounded-2xl animate-pulse" />
        )}
      </div>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {circuitsList.map((circuit, index) => (
          <LazyCircuitCard key={circuit.id} circuit={circuit} index={index} />
        ))}
      </div>
    </div>
  );
